...
WPA_SUP_PNAME="wpa_supplicant_s1g"
WPA_SUP_PIDFILE="/var/run/wpa_supplicant_s1g.$IFACE.pid"
WPA_COMMON_CTRL_IFACE="/var/run/wpa_supplicant_s1g"
...
if [ "$MODE" = "start" ] ; then
	...
	if [ -s "/etc/wpa_supplicant_s1g/driver.$IFACE" ]; then
		IF_WPA_DRIVER=$(cat "/etc/wpa_supplicant_s1g/driver.$IFACE")
	...
	# if we have passed the criteria, start wpa_supplicant_s1g
	...
	# if the interface socket exists, then wpa_supplicant_s1g was invoked successfully
